...
void Cylinder::CreateCylinder(float topRadius, float bottomRadius, float length, UINT slices, UINT stacks)
{
...
const Vector3D& Cylinder::GetPosition() const
{
	return m_position;
}

void Cylinder::SetDirection(const Vector3D& direction)
{
	m_direction = direction;
}

const Vector3D& Cylinder::GetDirection() const
{
	return m_direction;
}

void Cylinder::SetWireFrame(bool wireFrame)
{
...
void Cylinder::Render()
{
	...
	Vector3D target = Vector3D(m_position.x + m_direction.x, m_position.y + m_direction.y, m_position.z + m_direction.z);
